...
impl Album {
    ...
    pub fn discover(input_dir: &Path) -> Result<Vec<Album>> {
        ...
        let albums: Vec<Album> = albums_map
            .into_iter()
    ...
    pub fn validate(&self) -> bool {
        let has_flac = self
            .files
            .iter()
            .any(|f| f.extension().and_then(|e| e.to_str()) == Some("flac"));
        let has_mp3 = self
            .files
            .iter()
            .any(|f| f.extension().and_then(|e| e.to_str()) == Some("mp3"));

        if has_flac && has_mp3 {
            log::warn!(
                "Skipping album with mixed FLAC and MP3 files: {:?}",
                self.path
            );
            return false;
        }
